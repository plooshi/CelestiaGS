#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: AthenaKillFeedItem

#include "Basic.hpp"

#include "FortniteUI_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass AthenaKillFeedItem.AthenaKillFeedItem_C
// 0x0008 (0x02A0 - 0x0298)
class UAthenaKillFeedItem_C final : public UFortHUDElementWidget
{
public:
	class URichTextBlock*                         KillText;                                          // 0x0298(0x0008)(BlueprintVisible, ExportObject, ZeroConstructor, InstancedReference, IsPlainOldData, RepSkip, NoDestructor, PersistentInstance, HasGetValueTypeHash)

public:
	void SetText(const class FText& Text);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"AthenaKillFeedItem_C">();
	}
	static class UAthenaKillFeedItem_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UAthenaKillFeedItem_C>();
	}
};
static_assert(alignof(UAthenaKillFeedItem_C) == 0x000008, "Wrong alignment on UAthenaKillFeedItem_C");
static_assert(sizeof(UAthenaKillFeedItem_C) == 0x0002A0, "Wrong size on UAthenaKillFeedItem_C");
static_assert(offsetof(UAthenaKillFeedItem_C, KillText) == 0x000298, "Member 'UAthenaKillFeedItem_C::KillText' has a wrong offset!");

}

