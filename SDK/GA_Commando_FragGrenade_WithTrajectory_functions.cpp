...
void UGA_Commando_FragGrenade_WithTrajectory_C::SetupGrenade(float AimPitch, class UClass** Base_Grenade, float* Projectile_Speed, struct FFortGameplayEffectContainerSpec* Explosion_Gameplay_Spec, float* Projectile_Gravity_Scale)
{
	...
	if (Explosion_Gameplay_Spec != nullptr)
		*Explosion_Gameplay_Spec = move(Parms.Explosion_Gameplay_Spec);
...
void UGA_Commando_FragGrenade_WithTrajectory_C::SpawnFragGrenade(class UClass* Param_ProjectileClass, struct FVector& SpawnLocation, const struct FRotator& InputPin, const struct FFortGameplayEffectContainerSpec& EffectContainerSpecToApplyOnExplode, float InitialSpeed, float Param_GravityScale)
{
	...
	Parms.InputPin = move(InputPin);
	Parms.EffectContainerSpecToApplyOnExplode = move(EffectContainerSpecToApplyOnExplode);
	Parms.InitialSpeed = InitialSpeed;
	Parms.Param_GravityScale = Param_GravityScale;

	UObject::ProcessEvent(Func, &Parms);

	SpawnLocation = move(Parms.SpawnLocation);
}
...
void UGA_Commando_FragGrenade_WithTrajectory_C::SpawnLocationAndRotation(struct FVector* OutLoc, struct FRotator* OutRot) const
{
	...
	if (OutLoc != nullptr)
		*OutLoc = move(Parms.OutLoc);

	if (OutRot != nullptr)
		*OutRot = move(Parms.OutRot);
}
